...
void gather_kernel_times()
{
    ...
    if (count > 0) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            tracepoint(hsa_runtime, kernel_start_nm, "kernels_paul", record.kernel, name.c_str(), record.agent.handle, record.queue->id, record.time.start - init_timestamp);
            tracepoint(hsa_runtime, kernel_end_nm, "kernels_paul", record.kernel, name.c_str(), record.agent.handle, record.queue->id, record.time.end - init_timestamp);
        }
        delete[] records;
    }
}
